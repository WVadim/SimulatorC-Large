...
void ASNode::build_routing(bool build_weighted, bool build_payload) {
	auto graph = generator->GetGraph();
	if (build_payload) {
		vector<vector<int>> uw_paths = graph->ShortestPath(id, true, false);
		/*std::string filename = "path_ways_" + std::to_string(id);
		std::ofstream output;
		output.open(filename, std::ios::out);
		for (int i = 0; i < uw_paths.size(); i++) {
			output << id;
			for (int j = 0; j < uw_paths[i].size(); j++) {
				output << " " << uw_paths[i][j];
			}
			output << "\n";
		}
	...
	if (involved and build_weighted) {
		vector<vector<int>> w_paths = graph->ShortestPath(id, false, true);
